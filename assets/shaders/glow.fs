...
extern MY_HIGHP_OR_MEDIUMP vec2 glow;
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    tex = map(tex, black, white, trueblack, white);
	...
	if (glow.g > 0.0 || glow.g < 1.0) 
    {

        vec2 pixel_size = 1/image_details;
        number bloom_spread = 5;
        number iterations = 9;
        number bloom_intensity = 1.5;
        vec4 sum = vec4(0.0);
        for (int i = 0; i < iterations; ++i) {
            ...
            for (int j = 0; j < iterations; ++j)
            {
                ...
                number glowfactor = 1-SAT.b;
                // result.b = result.b * (1-SAT.b);
                result = RGB(result);
                h_sum.a = min(h_sum.a, result.a);
                h_sum.rgb += result.rgb * h_sum.a * glowfactor;
            }
            sum += h_sum / 9.0;
        }
